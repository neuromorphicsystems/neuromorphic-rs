...
pub enum Error {
    ...
    #[error(
        "short write ({requested} bytes requested, {written} bytes written)"
    )]
...
pub const DEFAULT_CONFIGURATION: Configuration =
    Device::PROPERTIES.default_configuration;
pub const DEFAULT_USB_CONFIGURATION: usb::Configuration =
    Device::DEFAULT_USB_CONFIGURATION;
...
impl device::Usb for Device {
    ...
    {
        let (handle, serial) =
            Self::handle_from_serial(event_loop.context(), serial)?;
        usb::assert_control_transfer(
            ...
            0x0300,                    // value
            0x0000,                    // index
            &[0x04, 0x03, 0x09, 0x04], // expected buffer
            TIMEOUT,
        )?;
        usb::assert_control_transfer(
            &handle,
            ...
            &[
                // expected buffer
                0x18, 0x03, b'C', 0x00, b'e', 0x00, b'n', 0x00, b't', 0x00,
                b'u', 0x00, b'r', 0x00, b'y', 0x00, b'A', 0x00, b'r', 0x00,
                b'k', 0x00, b's', 0x00,
            ],
            TIMEOUT,
        )?;
        ...
        usb::assert_control_transfer(
            ...
            // len = 50
            &[
                0x32, 0x03, b'S', 0x00, b'i', 0x00, b'l', 0x00, b'k', 0x00,
                b'y', 0x00, b'E', 0x00, b'v', 0x00, b'C', 0x00, b'a', 0x00,
                b'm', 0x00, b' ', 0x00, b'H', 0x00, b'D', 0x00, b' ', 0x00,
                b'v', 0x00, b'0', 0x00, b'3', 0x00, b'.', 0x00, b'0', 0x00,
                b'9', 0x00, b'.', 0x00, b'0', 0x00, b'0', 0x00, b'C', 0x00,
            ],
            TIMEOUT,
        )?;
        request(
        ...
        // Read default biases
        let mut chip_firmware_configuration =
        ...
        loop {
            let mut buffer =
                vec![0u8; Self::DEFAULT_USB_CONFIGURATION.buffer_length];
        ...
        Ok(Device {
            ...
            ring: usb::Ring::new(
                ...
                move |usb_error| {
                    error_flag
                        .store_error_if_not_set(Self::Error::from(usb_error));
                },
                move |overflow| {
    ...
    fn temperature_celsius(
        &self,
    ) -> Result<device::TemperatureCelsius, Self::Error> {
        let _guard = self
            .register_mutex
            .lock()
            .expect("register mutex is not poisoned");
...
fn update_configuration(
    ...
    {
        let previous_biases =
            previous_configuration.map(|configuration| &configuration.biases);
    ...
    if match previous_configuration {
        ...
        for offset in 0..((configuration.x_mask.len() as u32) * 2) {
            let register = TdRoiX {
                value: if (offset % 2) == 0 {
                    (configuration.x_mask[(offset / 2) as usize]
            ...
            let register = TdRoiY {
                value: if (offset % 2) == 0 {
                    let [byte2, byte3, _, _, _, _, _, _] = configuration.y_mask
                        [configuration.y_mask.len()
                            - 1
                            - (offset / 2) as usize]
                        .to_le_bytes();
                    if offset < (configuration.y_mask.len() as u32) * 2 - 2 {
                        let [_, _, _, _, _, _, byte0, byte1] = configuration
                            .y_mask[configuration.y_mask.len()
                            - 2
                            - (offset / 2) as usize]
                            .to_le_bytes();
                        u32::from_le_bytes([
                    ...
                    let [_, _, byte0, byte1, byte2, byte3, _, _] =
                        configuration.y_mask[configuration.y_mask.len()
                            - 2
                            - (offset / 2) as usize]
                            .to_le_bytes();
                    u32::from_le_bytes([
    ...
    if match previous_configuration {
        ...
        for offset in 0u32..64u32 {
            let code = if offset < 63 {
                (((configuration.pixel_mask[(offset / 3) as usize])
                    >> ((offset % 3) * 21))
                    & 0x1fffff) as u32
            } else {
                let mut code: u32 = 0;
                for bit in 0..21 {
                    code |=
                        ((configuration.pixel_mask[bit] >> 63) << bit) as u32;
                }
                code
            };
            if code == 0 {
